 ...
 * @subpackage Admin
 */
if ( ! class_exists( 'Beauty_Studio_Theme_Info' ) ) {
    class Beauty_Studio_Theme_Info{
        ...
        public function prepare_class() {
            ...
            $this->page_title    = isset( $this->config['page_title'] ) ? $this->config['page_title'] : esc_html__('Info','beauty-studio'). $this->theme_name;
            $this->menu_title    = isset( $this->config['menu_title'] ) ? $this->config['menu_title'] : esc_html__('Info','beauty-studio') . $this->theme_name;
            $this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__('Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.','beauty-studio'), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-info' ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-info' ) ) . '" class="button text-decoration-none">' . sprintf( __('Get started with %s','beauty-studio'), $this->theme_name ) . '</a></p>' );
        ...
        public function recommended_actions() {
            ...
            if ( ! empty( $recommended_actions ) ) {
                ...
                if ( $hooray ){
                    echo '<span class="hooray">' . esc_html__( 'Hooray! There are no recommended actions for you right now.', 'beauty-studio' ) . '</span>';
                    echo '<a data-action="reset" id="reset" class="reset-all" href="#">'.esc_html__('Show All Recommended Actions','beauty-studio').'</a>';
                }
                echo '</div>';
            }
        }
                    ...
                    foreach ( $recommended_plugins as $recommended_plugins_item ) {
                        ...
                        if ( ! empty( $recommended_plugins_item['slug'] ) ) {
                            ...
                            if ( ! empty( $info->author ) ) {
                                echo '<span class="separator">'.esc_html__('|','beauty-studio') .'</span>' . wp_kses_post( $info->author );
                            }

                            if ( ! empty( $info->name ) && ! empty( $active ) ) {
                                echo '<div class="action_bar ' . ( ( $active['needs'] !== 'install' && $active['status'] ) ? 'active' : '' ) . '">';
                                echo '<span class="plugin_name">' . ( ( $active['needs'] !== 'install' && $active['status'] ) ? esc_html__('Active:','beauty-studio') : '' ) . esc_html( $info->name ) . '</span>';
                                echo '</div>';
                        ...
                        if( !empty( $screenshot) ){
                            ...
                            if ( ! empty( $title ) ) {
                                ...
                                if ( ! empty( $download_link ) ) {
                                    echo '<a href="' . esc_url( $download_link ) . '" class="button button-primary install right">' . esc_html__( 'Download','beauty-studio' ) . '</a>';
                                }
                                if ( ! empty( $preview_link ) ) {
                                    echo '<a class="button button-secondary preview right" target="_blank" href="' . esc_url( $preview_link ) . '">' . esc_html__( 'Live Preview','beauty-studio' ). '</a>';
                                }
                                echo '</div>';
                                echo '</div>';
                            }
        ...
        public function free_pro() {
            ...
            if ( ! empty( $free_pro ) ) {
                ...
                if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
                    ...
                    echo '<tr>';
                    echo '<th></th>';
                    echo '<th>' . esc_html__( 'Beauty Studio','beauty-studio' ) . '</th>';
                    echo '<th>' . esc_html__( 'Beauty Studio Pro','beauty-studio' ) . '</th>';
                    echo '</tr>';
                    echo '</thead>';
                    echo '<tbody>';
                    ...
                    echo '<td></td>';
                    echo '<td colspan="2"><a href="https://www.acmethemes.com/themes/beauty-studio-pro/" target="_blank" class="button button-primary button-hero"> Beauty Studio Pro</a></td>';
                    echo '</tr>';
...
$config = array(
    // Page title.
    'page_title'               => esc_html__( 'Beauty Studio Info', 'beauty-studio' ),

    // Menu name under Appearance.
    'menu_title'               => esc_html__( 'Beauty Studio Info', 'beauty-studio' ),

    // Main welcome title
    'info_title'         => sprintf( esc_html__( 'Welcome to %s - ', 'beauty-studio' ), 'Beauty Studio' ),

    // Main welcome content
    'info_content'       => sprintf( esc_html__( '%s is now installed and ready to use. We hope the following information will help you. If you want to ask any query or just want to say hello, you can always contact us. We hope you enjoy it! ', 'beauty-studio' ), '<b>Beauty Studio</b>' ),
    ...
    'quick_links'                    => array(
        'theme_url'  => array(
                'text' => __('Theme Details','beauty-studio'),
                'url' => 'https://www.acmethemes.com/themes/beauty-studio/'
        ),
        'demo_url'  => array(
            'text' => __('View Demo','beauty-studio'),
            'url' => 'http://www.demo.acmethemes.com/beauty-studio/'
        ),
        'pro_url'  => array(
            'text' => __('View Pro Version','beauty-studio'),
            'url' => 'https://www.acmethemes.com/themes/beauty-studio-pro/'
        ),
        'rate_url'  => array(
            'text' => __('Rate This Theme','beauty-studio'),
    ...
    'tabs' => array(
        ...
        'free_pro'         => esc_html__( 'Free VS PRO', 'beauty-studio' ),
    ),
    ...
    'getting_started' => array(
        'first' => array(
            ...
            'link_url' => 'http://www.doc.acmethemes.com/beauty-studio/',
            'is_button' => false,
        ...
        'sixth' => array (
	        ...
	        'desc' => esc_html__( 'If you are using this theme for your online store or shop, do not forget to install WooCommerce Plugin','beauty-studio' ),
	        'link_title' => esc_html__( 'Install Recommended Plugins','beauty-studio' ),
	        'link_url' => '?page=beauty-studio-info&tab=recommended_plugins',
	        'is_button' => true,
	        'recommended_actions' => false,
	        'is_new_tab' => false
        ),
    ...
    'recommended_actions'        => array(
        'demo-content' => array(
            ...
            'desc' => sprintf( esc_html__( 'For the demo content installation, install the Acme Demo Setup plugin and other Recommended plugins, download demo content from %1$s here %2$s. Then go to "Acme Demo Setup" and import demo. ', 'beauty-studio' ), '<a target="_blank" href="http://www.doc.acmethemes.com/beauty-studio/#DummyData" >','</a>' ),
            'id' => 'acme-demo-setup',
        ...
        'front-page' => array(
            ...
            'desc' => sprintf( esc_html__( 'Select A static page than Front page and Posts page to enable Slider and Home Main Content Area %1$s Static Front Page %2$s ', 'beauty-studio' ), '<p><a target="_blank" href="'.esc_url( admin_url( 'customize.php' ) ).'?autofocus[section]=static_front_page'.'" class="button button-primary">','</a></p>' ),
            'id' => 'front-page',
    ...
    'faq'      => array(
        'first' => array (
            ...
            'desc' => sprintf( esc_html__( 'For the demo content installation, install the Acme Demo Setup plugin and other Recommended plugins, download demo content from %1$s here %2$s. Then go to "Acme Demo Setup" and import demo. ', 'beauty-studio' ), '<a target="_blank" href="http://www.doc.acmethemes.com/beauty-studio/#DummyData" >','</a>' ),
            'button_label' => esc_html__( 'Download Demo Content','beauty-studio' ),
            'button_link' => esc_url( 'http://www.doc.acmethemes.com/beauty-studio/#DummyData' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'second' => array (
        ...
        'third' => array (
            ...
            'icon' => 'dashicons dashicons-sos',
            'desc' => esc_html__( 'Please visit the blog post to know more about it.','beauty-studio' ),
    ...
    'support_content'      => array(
        'first' => array (
            ...
            'button_link' => esc_url( 'https://www.acmethemes.com/supports/forum/beauty-studio/' ),
            'is_button' => true,
            'is_new_tab' => true
        ),
        'second' => array(
            ...
            'button_link' => 'http://www.doc.acmethemes.com/beauty-studio/',
            'is_button' => false,
            'is_new_tab' => true
        ),
        'third' => array(
            'title' => esc_html__( 'Need more features?','beauty-studio' ),
            'icon' => 'dashicons dashicons-screenoptions',
            'desc' => esc_html__( 'Upgrade to PRO version for more exciting features and Priority Support.','beauty-studio' ),
            'button_label' => esc_html__( 'View Pro Version','beauty-studio' ),
            'button_link' => 'https://www.acmethemes.com/themes/beauty-studio-pro/',
            'is_button' => true,
            'is_new_tab' => true
        ),
        'fourth' => array(
            'title' => esc_html__( 'Got sales related question?','beauty-studio' ),
            'icon' => 'dashicons dashicons-cart',
            'desc' => esc_html__( "Have any query before purchase, you are more than welcome to ask.",'beauty-studio' ),
            'button_label' => esc_html__( 'Pre-sale Question?','beauty-studio' ),
    ...
    'free_pro' => array(
	    array(
		    'title'=> esc_html__( 'Custom Widgets', 'beauty-studio' ),
		    'desc' => esc_html__( 'All added custom widgets are ready for page builder plugin', 'beauty-studio' ),
		    'free' => esc_html__('9+ Available','beauty-studio'),
		    'pro'  => esc_html__('16+ Available','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Pricing Plans', 'beauty-studio' ),
		    'desc' => esc_html__( 'Pricing Plans Widgets', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'Gallery Filter', 'beauty-studio' ),
		    'desc' => esc_html__( 'Galley/Portfolio filter ', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'Google Map', 'beauty-studio' ),
		    'desc' => esc_html__( 'Map with advanced options', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'Advanced WooCommerce Widget', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'Widget Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Every widgets have multiple options to customize section', 'beauty-studio' ),
		    'free' => esc_html__('Limited','beauty-studio'),
		    'pro'  => esc_html__('Multiple Advanced Options','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Google Fonts', 'beauty-studio' ),
		    'desc' => esc_html__( 'Google fonts options for changing the overall site fonts ', 'beauty-studio' ),
		    'free' => 'yes',
		    'pro'  => esc_html__('Unlimited +','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Color Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Color options for changing overall site colors, Contat us', 'beauty-studio' ),
		    'free' => esc_html__('Limited','beauty-studio'),
		    'pro'  => esc_html__('Almost All parts','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Header Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Customize the header section as your need.', 'beauty-studio' ),
		    'free' => esc_html__('Limited','beauty-studio'),
		    'pro'  => esc_html__('Multiple Options','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Breaking News Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Manage your breaking news section on header', 'beauty-studio' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'Menu Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Lots of menu options are available to manage the menu section. Appearance > Menus', 'beauty-studio' ),
		    'free' => esc_html__('Limited','beauty-studio'),
		    'pro'  => esc_html__('Multiple Options','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Featured Selection', 'beauty-studio' ),
		    'desc' => esc_html__( 'This is slider section. Check every options there','beauty-studio' ),
		    'free' => esc_html__('Unlimited Page','beauty-studio'),
		    'pro'  => esc_html__('Unlimited Page, Custom or Third Party','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Featured Section Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Options to manage the sliders', 'beauty-studio' ),
		    'free' => esc_html__('Limited','beauty-studio'),
		    'pro'  => esc_html__('Multiple Options','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Sticky Sidebar', 'beauty-studio' ),
		    'desc' => esc_html__( 'Enable/Disable sticky sidebars', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'Blog/Archive Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Various Options are available for the blog and archive page management. Check on Appearance > Customize > Layout/Design Options.', 'beauty-studio' ),
		    'free' => esc_html__('Limited','beauty-studio'),
		    'pro'  => esc_html__('Manage Almost Every Entity','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Footer Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Manage the every entity related to footer section.', 'beauty-studio' ),
		    'free' => esc_html__('Limited','beauty-studio'),
		    'pro'  => esc_html__('Multiple Options','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Social Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Options to manage the social icons like facebook, instagram, linkedin etc.', 'beauty-studio' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'Theme Credit Link', 'beauty-studio' ),
		    'desc' => esc_html__( 'Options to disable to Powered by text on footer.', 'beauty-studio' ),
		    'free' => esc_html__('No Options To Disable','beauty-studio'),
		    'pro'  => esc_html__('Enable/Disable','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Own Credit Link', 'beauty-studio' ),
		    'desc' => esc_html__( 'Write your own credit text.', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => esc_html__('Enable/Disable','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Social Icons', 'beauty-studio' ),
		    'desc' => esc_html__( 'Enter the URL for social icons. ', 'beauty-studio' ),
		    'free' => esc_html__('Unlimited with multiple Options','beauty-studio'),
		    'pro'  => esc_html__('Unlimited with multiple Options','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Intro Loader Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'It will load before the site loaded completely.', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => esc_html__('Multiple Options','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Related Post Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Related post will appear on single post of each blog article', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => esc_html__('Multiple Options','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Author Info Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'This will appear on single post.', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => esc_html__('Multiple Options','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Feature Image Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Featured image options for the single page and post.', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => esc_html__('Manage As Your Need','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Navigation Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Post Navigation means, previous post and next post. Check on single post.', 'beauty-studio' ),
		    'free' => esc_html__('Default','beauty-studio'),
		    'pro'  => esc_html__('Advanced','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Post Comment Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Manage the comment section on single post.', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => esc_html__('Multiple Options','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Show/Hide Comment', 'beauty-studio' ),
		    'desc' => esc_html__( 'Options to show hide comments on page and post.', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'Breadcrumb', 'beauty-studio' ),
		    'desc' => esc_html__( 'Advanced options for manage the Breadcrumb.', 'beauty-studio' ),
		    'free' => 'yes',
		    'pro'  => esc_html__('Advanced','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Video Post On Featured', 'beauty-studio' ),
		    'desc' => esc_html__( 'Create your video post easily from Youtube and Vimeo video.', 'beauty-studio' ),
		    'free' => 'no',
		    'pro'  => esc_html__('Add Own Video On Featured','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Single Post Options', 'beauty-studio' ),
		    'desc' => esc_html__( 'Manage every entity of single post.', 'beauty-studio' ),
		    'free' => esc_html__('Limited','beauty-studio'),
		    'pro'  => esc_html__('Multiple Options','beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Featured Image Size', 'beauty-studio' ),
		    'desc' => esc_html__( 'Select the size of the featured image on the single post.', 'beauty-studio' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'Sidebars', 'beauty-studio' ),
		    'desc' => esc_html__( 'Global and Individual sidebars are available. You can easily orveride it from single page/post.', 'beauty-studio' ),
		    'free' => esc_html__('Advanced Global And Individuals', 'beauty-studio'),
		    'pro'  => esc_html__('Advanced Global And Individuals', 'beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Menu and Logo Location', 'beauty-studio' ),
		    'desc' => esc_html__( 'Position of menu and logo section on header.', 'beauty-studio' ),
		    'free' => esc_html__('Default', 'beauty-studio'),
		    'pro'  => esc_html__('3 Position Left, Right and Middle', 'beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Popup Widget on Header', 'beauty-studio' ),
		    'desc' => esc_html__( 'Options to popup widget via Popup Widget Area on Header Top Section.', 'beauty-studio' ),
		    'free' => esc_html__('Add your required Widget', 'beauty-studio'),
		    'pro'  => esc_html__('Add your required Widget', 'beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'Multiple Header Type', 'beauty-studio' ),
		    'desc' => esc_html__( 'Different Types of Header.', 'beauty-studio' ),
		    'free' => esc_html__('Default', 'beauty-studio'),
		    'pro'  => esc_html__('3 Options', 'beauty-studio'),
	    ),
	    array(
		    'title'=> esc_html__( 'WooCommerce', 'beauty-studio' ),
		    'desc' => esc_html__( 'Create WooCommerce shop easily with the help of WooCommerce plugin.', 'beauty-studio' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'Translation', 'beauty-studio' ),
		    'desc' => esc_html__( 'Both theme are translation ready.', 'beauty-studio' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'SEO', 'beauty-studio' ),
		    'desc' => esc_html__( 'Developed with high skilled SEO tools.', 'beauty-studio' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'Widget Area On Footer', 'beauty-studio' ),
		    'desc' => esc_html__( 'Change the number of footers on footer area as you need.', 'beauty-studio' ),
		    'free' => 'yes',
		    'pro'  => 'yes',
	    ),
	    array(
		    'title'=> esc_html__( 'Support Forum', 'beauty-studio' ),
		    'desc' => esc_html__( 'Highly dedicated support team are assigned for your help. Try this today.', 'beauty-studio' ),
		    'free' => esc_html__('Second', 'beauty-studio'),
		    'pro'  => esc_html__('Dedicated With High Priority', 'beauty-studio'),
	    )
    )
);

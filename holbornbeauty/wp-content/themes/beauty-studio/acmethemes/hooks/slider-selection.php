 ...
 *
 */
if ( !function_exists('beauty_studio_default_slider') ) :
    function beauty_studio_default_slider(){
        $bg_image_style = '';
        if ( get_header_image() ) :
                    ...
                    <div class="slider-content <?php echo $text_align;?>">
                        <div class="container">
                            <div class="banner-title <?php echo $animation1;?>">
                                <?php esc_html_e('Beauty Studio','beauty-studio' );?>
                            </div>
                            <div class="image-slider-caption <?php echo $animation2;?>">
                                <?php esc_html_e('Building the World Together','beauty-studio' );?>
                            </div>
                        </div>
                    </div>
                </div>
...
function beauty_studio_slider_from_page(){
	global $beauty_studio_customizer_all_values;

	$beauty_studio_slides_data = json_decode( $beauty_studio_customizer_all_values['beauty-studio-slides-data'] );
	$beauty_studio_feature_slider_text_align = $beauty_studio_customizer_all_values['beauty-studio-feature-slider-text-align'];

	$beauty_studio_feature_slider_enable_animation = $beauty_studio_customizer_all_values['beauty-studio-feature-slider-enable-animation'];
	$beauty_studio_feature_slider_image_only = $beauty_studio_customizer_all_values['beauty-studio-feature-slider-display-title'];
	$beauty_studio_feature_slider_image_excerpt = $beauty_studio_customizer_all_values['beauty-studio-feature-slider-display-excerpt'];
	$beauty_studio_fs_image_display_options = $beauty_studio_customizer_all_values['beauty-studio-fs-image-display-options'];

	$post_in = array();
	$slides_other_data = array();
	if( is_array( $beauty_studio_slides_data ) ){
		foreach ( $beauty_studio_slides_data as $slides_data ){
	...
	if( !empty( $post_in )) :
		...
		$slider_query = new WP_Query( $beauty_studio_child_page_args );
		/*The Loop*/
		if ( $slider_query->have_posts() ):
			?>
            <div class="image-slider-wrapper home-fullscreen <?php echo esc_attr( $beauty_studio_fs_image_display_options ); ?>">
                <div class="featured-slider">
					<?php
					$slider_index = 1;
					$text_align = '';
					...
					$bg_image_style = '';
					if( 'alternate' != $beauty_studio_feature_slider_text_align ){
						$text_align = $beauty_studio_feature_slider_text_align;
					}
					if( 1 == $beauty_studio_feature_slider_enable_animation ){
					...
					while( $slider_query->have_posts() ):$slider_query->the_post();
						...
						if( 'full-screen-bg' == $beauty_studio_fs_image_display_options ){
                        ...
                        <div class="item" style="<?php echo $bg_image_style; ?>">
							<?php
							if( 'responsive-img' == $beauty_studio_fs_image_display_options ){
                            ...
                            <div class="slider-content <?php echo esc_attr( $text_align );?>">
                                <div class="container">
									<?php
									if( 1 == $beauty_studio_feature_slider_image_only ){
										?>
                                        <div class="banner-title <?php echo esc_attr( $animation1 );?>"><?php the_title()?></div>
										<?php
									}
									if( 1 == $beauty_studio_feature_slider_image_excerpt ){
										?>
				...
				if( 'absolute' == $beauty_studio_feature_info_display_options ){
...
if ( ! function_exists( 'beauty_studio_feature_slider' ) ) :

    function beauty_studio_feature_slider( ){
	    beauty_studio_slider_from_page();
    }
endif;
add_action( 'beauty_studio_action_feature_slider', 'beauty_studio_feature_slider', 0 );